...
resource "aws_lambda_function" "sqs_fun" {
  filename      = "${path.module}/code/lamc.py.zip"
  function_name = "MySQSlambda"
  role          = aws_iam_role.sqs-role.arn
  handler       = "lamc.lambda_handler"
  runtime       = "python3.11"
  environment {
    variables = {
...
data "archive_file" "zip_the_python_code" {
 ...
 output_path = "${path.module}/code/lamc.py.zip"
}
...
# resource "aws_lambda_event_source_mapping" "ev-source_mapping" {
